...
rsync -avz --exclude '.git' --exclude 'venv' --exclude '__pycache__' --exclude '.terraform' --exclude '*.tfstate*' --exclude 'terraform.tfvars' --exclude 'bootstrap' \
  ...
  ./ "ubuntu@${HOST}:/opt/voxfoxbot/"

ssh -i "$KEY" "ubuntu@${HOST}" 'cd /opt/voxfoxbot && source venv/bin/activate && sudo systemctl restart voxfoxbot || echo "no systemd service yet — start manually"'
